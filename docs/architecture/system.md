...
| `audio_artifacts` | built | migration 025 — provenance for every downloaded earnings-call audio (source URL, sha256, vendor, captured_at, deleted_at). Persists after the audio binary is deleted from disk so we can re-download by URL + verify by hash. See `docs/architecture/asr_transcripts_ingest_plan.md`. |
| `asr_transcripts` | built | migration 025 — back-pointer linking an `artifacts` row to the `audio_artifacts` row + ASR model that produced it. Keyed by `(audio_artifact_id, model, model_version)` so re-runs with newer models append cleanly. The transcript text itself stays on `artifacts` + `artifact_text_units` exactly like FMP transcripts. |
| `speaker_voiceprints` | built | migration 025 — pyannote 3.1 embeddings (192-dim float8[]) per `(company_id, person_name, role)`. Once enrolled, recurring execs (CEO/CFO) auto-identify on every future call via cosine match. Supersession via `superseded_at` + `superseded_by`. |
| `speaker_segments` | built | migration 025 — diarization output per ASR transcript: who spoke when (start_ms, end_ms, raw_speaker_label) plus voiceprint-match results. Joinable to `artifact_text_units` by text offset. |
