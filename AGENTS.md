...
- NVDA full pipeline tested: 12 filings, 262/264 golden eval fields match (2 operating lease diffs under investigation)
- Linkbase-based pipeline complete: Stage 1 + Stage 2 + Stage 3 + calculate.py all updated
- Cost per filing: ~$3/filing (Sonnet), includes Stage 1 + Stage 2 + any retries
- XBRL fact completeness check built: compares AI output against filing's own XBRL, targeted retry for gaps
- Auto HTML sizing: full HTML for small filings, stripped + retry for large filings
- Linkbase download + parse working for all 9 companies (194 filings total)
- Old template-based pipeline tagged as `v1-template-prompt` for rollback
- Deterministic pipeline archived in `deterministic-flow/`
